...
E_y(x) = \left\{
\begin{aligned}
&A \cos \left( k_{x,1} a - \phi \right) \exp{-k_{x,0} (x - a)}, \quad &x > a \\
&A \cos \left( k_{x,1} x - \phi \right), \quad &|x| \leq a \\
&A \cos \left( k_{x,1} a + \phi \right) \exp{k_{x,s} (x + a)}, \quad &x < -a
\end{aligned}
\right.
&&
\end{flalign*}
...
$$
\begin{alinged}
E_{y,i}(x) &= A_i\exp{ik_{x,i}(x-a_i)} + B_i\exp{-ik_{x,i}(x-a_i)} \\
\text{with} \\
...
<img src="/study_img/swg/nprofile.png" alt="refractive-index profile" style="width: 100%; height: auto;"/>
