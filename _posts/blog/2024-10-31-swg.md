...
For $\vec{\Psi} \in \left\{ \vec{E}, \vec{H} \right\}$.
Since our solution has harmonic time dependence $\vec{\Psi}(\vec{r},t)=\vec{\Psi}(\vec{r})exp(-i\omega t)$ (or using separation of variables)