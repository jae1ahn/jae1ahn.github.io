...
<script>
  new p5((p) => {
    ...
    const dataPath = "/study_data/cdc/wgm_radial.json"; // JSON 경로

    // 데이터 로드
    p.preload = function () {
      wgmData = p.loadJSON(
        dataPath,
        () => console.log("WGM JSON loaded"),
        (err) => console.error("WGM JSON load failed", err)
      );
    };
    ...
    // seismic colormap: E in [-1,1] -> [r,g,b]
    function seismicColor(E) {
      let v = Math.max(-1, Math.min(1, E)); // clamp to [-1,1]
      const t = (v + 1) / 2; // [0,1]

      let r, g, b;
      if (t < 0.5) {
        // 파랑(0,0,255) -> 흰색(255,255,255)
        const a = t / 0.5;
        r = 255 * a;
        g = 255 * a;
        b = 255;
      } else {
        // 흰색(255,255,255) -> 빨강(255,0,0)
        const a = (t - 0.5) / 0.5;
        r = 255;
        g = 255 * (1 - a);
        b = 255 * (1 - a);
    ...
    p.draw = function () {
      // JSON 아직 안 읽혔으면 로딩 화면만
      if (!wgmData) {
        p.background(0);
        p.fill(255);
        p.textAlign(p.CENTER, p.CENTER);
        p.text("Loading WGM data...", p.width / 2, p.height / 2);
        return;
      }
      ...
      // 공진기 경계 그리기 (r=R) -> 반지름 w/4
      p.noFill();
      p.stroke(255);
      p.strokeWeight(1);
      const radiusPx = w / 4;
      p.ellipse(w / 2, h / 2, radiusPx * 2, radiusPx * 2);
